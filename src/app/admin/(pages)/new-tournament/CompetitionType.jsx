...
const PremiumCompetitionTypeSelector = ({
  competitionTypes,
  selectedType,
  onChange
}) => {
  ...
  useEffect(() => {
    if (selectedType) {
      const index = Object.values(competitionTypes).findIndex(data => data.title === selectedType);
      if (index !== -1) {
  ...
  // Handle selection
  const handleSelect = (e, index, title) => {
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {Object.entries(competitionTypes).map(([type, data], index) => {
            const isActive = activeIndex === index;
            const { color, icon: GameIcon } = getGameAttributes(data.title);
            
            return (
              <button
                key={type}
                type="button"
                onClick={(e) => handleSelect(e, index, data.title)}
                className={`
              ...
              >
                ...
                <div className="absolute inset-0 transition-transform duration-500 group-hover:scale-105 z-0">
                  ...
                  <div className={`absolute inset-0 transition-opacity duration-300 z-1
                    ${isActive 

...
const BattleRoyale = ({ tournamentId }) => {
    ...
    const fetchLeaderboardData = async () => {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          ...
          const response = await axios.get(`${backendUrl}/api/get_battle_royale_leaderboard.php?tournament_id=${tournamentId || 2}`);
  ...
  className={`group relative overflow-hidden flex items-center text-white py-2.5 px-5 angular-cut shadow-lg transition-all duration-300 

  }`}
>
  {/* Background shine effect */}
  <div className="absolute inset-0 w-full h-full bg-white/10 transform -skew-x-20 -translate-x-full group-hover:translate-x-full transition-transform duration-700"></div>
  
  {/* Icon container with animation */}
  <div className={`relative z-10 flex items-center justify-center w-7 h-7 mr-3 rounded-full ${
    isAdminModalOpen 
      ? 'bg-white/10' 
      : 'bg-primary/20'
  } transition-all duration-300 group-hover:scale-110`}>
    {isAdminModalOpen ? (
      <FaUnlock className="text-white transition-all duration-300 group-hover:rotate-12" />
    ) : (
      <FaLock className="text-white transition-all duration-300 group-hover:-rotate-12" />
    )}
  </div>
  
  {/* Text with slide transition */}
  <span className="relative z-10 text-sm font-semibold uppercase hover:text-primary tracking-wider">
    {isAdminModalOpen ? 'Exit Admin Mode' : 'Leaderboard Management'}
  </span>
  
  {/* Angular corner accent */}
  <div className={`absolute top-0 right-0 border-t-8 border-r-8 ${
    isAdminModalOpen 
      ? 'border-red-400' 
      : 'border-primary/60'
  ...
  return (
    <div className="min-h-screen w-full text-white">
      ...
      <table className="w-full border-collapse table-fixed">
        ...
        { teams.length > 0 && teams.slice(0, 10).map((team, index) => (
            <tr key={team.participant_id} className="angular-cut hover:bg-gray-800/30 cursor-pointer relative">
              ...
              <td colSpan={6} className="absolute inset-0 m-0 p-0 border-none">
                <div className="absolute inset-0 z-0">
                  ...
                  <div
                    ...
                    style={{
                      backgroundImage: team.participant_image ? `url(${process.env.NEXT_PUBLIC_BACKEND_URL}${team.participant_image})` : '',
